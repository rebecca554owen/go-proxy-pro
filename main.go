...
// Version 在构建时通过 -ldflags 注入（例如：-X main.Version=v1.0.0）
var Version = "dev"
...
func main() {
	...
	// 打印启动横幅和系统信息
	log.Info("AiProxy 服务启动 | 版本: %s | 系统: %s/%s | CPU: %d核 | Go: %s | PID: %d | 工作目录: %s",
		Version, runtime.GOOS, runtime.GOARCH, runtime.NumCPU(), runtime.Version(), os.Getpid(), getWorkDir())
