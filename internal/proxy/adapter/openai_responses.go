/*
 * 文件作用：OpenAI Responses API 适配器，处理 Codex/Responses 接口转发
 * 负责功能：
 *   - /responses API 请求转发
...
func (a *OpenAIResponsesAdapter) SendStream(ctx context.Context, account *model.Account, req *Request, writer io.Writer) (*StreamResult, error) {
	...
	// 发送请求 - 使用流式 HTTP 客户端（10分钟超时）
	// chatgpt.com 使用 Chrome TLS
	var client *http.Client
	if NeedsChromeTLS(targetURL) {
		client = GetChromeTLSClient(account)
	} else {
	...
	if resp.StatusCode != http.StatusOK {
		result, err := a.handleErrorResponse(resp, account, log)
		if err != nil {
			// 发送 SSE 错误事件给客户端
			a.sendSSEError(writer, fmt.Sprintf("upstream_error_%d", resp.StatusCode), err.Error())
		}
		return result, err
	}
	...
	return a.processStreamResponse(ctx, resp, writer, log)
}
...
func (a *OpenAIResponsesAdapter) processStreamResponse(ctx context.Context, resp *http.Response, writer io.Writer, log *logger.Logger) (*StreamResult, error) {
	result := &StreamResult{
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "data: ") {
			...
			if data == "[DONE]" {
				...
				if writeErr != nil {
					log.Warn("OpenAI Responses Stream 写入 [DONE] 失败: %v", writeErr)
				}
				if hasFlusher {
					flusher.Flush()
				}
	...
	if err := scanner.Err(); err != nil {
		...
		log.Error("OpenAI Responses Stream 读取上游错误: %v", err)
...
func (a *OpenAIResponsesAdapter) sendSSEError(writer io.Writer, errorType, message string) {
	flusher, hasFlusher := writer.(http.Flusher)
